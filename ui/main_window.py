...
from tools.pxe.pxe_ui import PXEPage
...
class FeatureToolsPage(QWidget):
    ...
    def init_ui(self):
        ...
        pxe_card = ToolCard("network-install", "网络装机", "PXE网络启动服务，支持批量网络装机", "#4CAF50")
        pxe_card.clicked.connect(self.open_pxe_tool)
        grid_layout.addWidget(pxe_card, 3, 0)
    ...
    def open_pxe_tool(self):
        from core.local_client import LocalClient
        dialog = QDialog(self)
        dialog.setWindowTitle("网络装机")
        dialog.setMinimumSize(1000, 700)
        dialog.resize(1000, 700)
        dialog.setStyleSheet(f"background-color: {BACKGROUND_MAIN};")
        
        layout = QVBoxLayout(dialog)
        pxe_page = PXEPage(LocalClient())
        layout.addWidget(pxe_page)
